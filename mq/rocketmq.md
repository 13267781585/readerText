...
## 延迟消息
RocketMQ支持发送延迟消息，仅支持内置预设值的延迟时间间隔的延迟消息；1s、 5s、 10s、 30s、 1m、 2m、 3m、 4m、 5m、 6m、 7m、 8m、 9m、 10m、 20m、 30m、 1h、 2h；

### 原理
* 生产者设置消息delayLevel延迟级别，消息属性DELAY中存储了对应了延时级别
* broker端收到消息后，判断延时消息延迟级别，如果大于0，则备份消息原始topic，queueId，并将消息topic改为延时消息队列特定topic(SCHEDULE_TOPIC)，queueId改为延时级别-1
* mq为每一个延迟级别单独设置一个定时器，定时(每隔1秒)拉取对应延迟级别的消费队列
...
[RocketMQ延迟消息实现原理和源码分析](https://juejin.cn/post/6997370258507956232)

# Redis多线程
redis处理命令仍然是单线程，只是读写加入了多线程，充分利用了多cpu的优势。主要有两个地方使用了多线程，一个是使用多线程进行io读写；一个是使用多线程处理后台的异步任务，例如释放过期对象、清空某个库数据、清除太大的lua脚本字典等。

## 多线程处理io
### 主函数处理流程
* `beforeSleep()->handleClientsWithPendingReadsUsingThreads()`，分配`clients_pending_read`队列中的读任务给多线程，线程各自读取数据后返回主线程，主线程执行命令。
* `beforeSleep()->handleClientsWithPendingWritesUsingThreads()`，分配`clients_pending_write`队列中的写任务给多线程，多线程写入完成后返回主线程。
* io多路复用休眠监听活跃事件，返回后根据事件的读写类型，执行对于回调函数，做一些准备工作(创建客户端，更新标志等)，加入对应`clients_pending_read`或`clients_pending_write`队列。
* 执行时间事件
* 重复上述流程
```c

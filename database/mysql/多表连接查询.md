...
2. 查询成本计算
    ...
    * 读取符合条件关键字
    需要估算查询范围内记录的数量，具体步骤如下：
        ...
        ii. 如果 区间最左记录 和 区间最右记录 相隔不太远（在 MySQL 5.7.21 这个版本里，只要相隔不大于10个页面即可），可以通过类加数据页中头部当前页中记录总数得出精确的满足 key2 > 10 AND key2 < 1000 条件的二级索引记录条数。否则只沿着 区间最左记录 向右读10个页面，计算平均每个页面中包含多少记录，然后用这个平均值乘以 区间最左记录 和 区间最右记录 之间的页面数量。
        如何估计两个页面之间页面数量？
    ...
    * 总成本
    ...
    3. idx_key1 索引查询
        * 二级索引记录数的预估
        ...
        idx_key1 对应的搜索条件是： key1 IN ('a', 'b', 'c')，相当于三个范围区间，['a', 'a']->35条，['b', 'b']->44条，['c', 'c']->39条。单点数量不超过eq_range_index_dive_limit设定的值，使用index dive方式分别统计区间的记录数相加。
        ![42](.\image\42.jpg)
